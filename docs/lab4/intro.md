...
在LoongArch32版本的uCore中，其磁盘是通过一块内存进行的模拟，并无真实的磁盘控制器硬件。因此整个文件系统的磁盘数据其实运行在内存中。在编译uCore时，makefile会调用`tools/mksfs`在你的计算机上完成SFS文件系统的格式化以及文件为镜像，并链接到`ucore-kernel-initrd`这一个可执行文件中。
